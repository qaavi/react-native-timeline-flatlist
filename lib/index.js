...
export default class Timeline extends PureComponent {
  ...
  _renderItem({ item, index }) {
    ...
    switch (this.props.columnFormat) {
      ...
      case "two-column":
        content =
          (item.payment_type && item.payment_type === "CR")  ? (
            <View style={[styles.rowContainer, this.props.rowContainerStyle]}>
              {this.renderTime(item, index)}
              {this.renderEvent(item, index)}
              {this.renderCircle(item, index)}
    ...
    switch (this.props.columnFormat) {
      ...
      case "two-column":
        timeWrapper = {
          flex: 1,
          alignItems:
            (rowData.payment_type && rowData.payment_type === "CR") 
              ? "flex-end"
              : "flex-start"
        };
        break;
    ...
    return (
      <View style={timeWrapper}>
        <View style={[styles.timeContainer, this.props.timeContainerStyle]}>
          ...
          >
            {moment(rowData.date).format('MMMM Do YYYY')}
  ...
  _renderDetail(rowData, rowID) {
    const PKR = (value) => currency(value, { symbol: 'Rs.', precision: 0 })
    const { isAllowFontScaling } = this.props;
    let description;
    if (typeof rowData.notes === "string") {
      description = (
        <Text
          style={[
            styles.description,
    ...
    } else if (typeof rowData.notes === "object") {
    ...
    return (
      ...
      <View style={styles.container}>
        <Text
          ...
          allowFontScaling={isAllowFontScaling}
        >
          {PKR(rowData.amount).format(true) + ' - ' + rowData.type}
        </Text>
        {description}
      </View>
    );
